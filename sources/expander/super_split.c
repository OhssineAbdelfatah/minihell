    ...
    while (*arg) {
        if (*arg == '"') {
            if (!in_single_quotes) {
                in_double_quotes = !in_double_quotes;
                if (in_double_quotes) {
                    start = arg + 1;  // Exclude the opening double quote
                } else {
                    char *word = ft_strndup(start, arg - start);  // Exclude the closing double quote
            ...
            if (!in_double_quotes) {
                ...
                if (in_single_quotes) {
                    ...
                    words[index] = ft_strndup(start, arg - start + 1);
                    index++;
                }
            }
        } else if (!in_single_quotes && !in_double_quotes) {
            if (!in_word) {
                in_word = true;
                start = arg;
            } else if (*(arg + 1) == '\0' || *(arg + 1) == '"' || *(arg + 1) == '\'') {
                char *word = ft_strndup(start, arg - start + 1);
                char *expanded_word = expand_env_vars(word, myenv);
                free(word);
                words[index] = expanded_word;
                index++;
